    ...
    try {
      switch (msg.id) {
        ...
        case msgTo(MessageType::INVITE_USER): {
          ...
          // 取出外呼信息
          std::string inviteInfo = std::any_cast<std::string>(msg.data);
          if (inviteInfo.empty()) {
            W_LOG("[VisionCentralContoller::update] invite info is empty!");
            break;
          }

          // 调用xxx方法通知中控器向信令发起外呼请求
          callback_->InviteUser(inviteInfo);
          break;
        }
        ...
        case msgTo(MessageType::SHARE_SCREEN_INFO): {
          std::shared_ptr<StreamScreen> point = std::dynamic_pointer_cast<StreamScreen>(streamWnd);
          if (!point) return;
          point->setShareList(std::any_cast<std::map<int, std::string>>(msg.data), 1);
          break;
        }
        case msgTo(MessageType::SHARE_WINDOW_INFO): {
          std::shared_ptr<StreamScreen> point = std::dynamic_pointer_cast<StreamScreen>(streamWnd);
          if (!point) return;
