...
namespace rtcengine {
	...
	bool RtcConnectEngine::scheduleMeeting(VideoMcu videoMcu_, AudioMcu audioMcu_, VideoCodecType videoType_, AudioCodecType audioType_)
	{
		I_LOG("videoMcu: {} audioMcu: {} videoCodecType: {} audioCodecType: {}", videoMcu_, audioMcu_, videoType_, audioType_);
		if (videoMcu_ == 0) {
			if (audioMcu_ == 0) {
				mcu = 0;
			}
		...
		if (videoType_ == VideoCodecType::H264) {
			videoType = "H264";
		}
		else if (videoType_ == VideoCodecType::VP9) {
			videoType = "VP9";
		}
		if (audioType_ == AudioCodecType::PCMA) {
			audioType = "PCMA";
		}
		else if (audioType_ == AudioCodecType::OPUS) {
			audioType = "OPUS";
		}
		Message scheduleReq;
		scheduleReq.set_from(userInfo.userId);
		scheduleReq.set_meth("CREATE");
		scheduleReq.set_cseq(cseq++);
		scheduleReq.set_call_id(std::to_string(rand()));
		scheduleReq.set_mcuId(mcu);
		scheduleReq.set_videoformat(videoType);
		scheduleReq.set_audioformat(audioType);
		scheduleReq.set_isresponse(false);
		I_LOG("schedule Req: {}", scheduleReq.js.dump(4));
		oatpp::String scheduleJson = oatpp::String(scheduleReq.js.dump());
		sendSocket(scheduleJson);
		return true;
	}
	...
	bool RtcConnectEngine::exitMeeting() {
		...
		videoEngine->stopStatsCollection();
		videoEngine->close();
		audioEngine->close();
	...
	void RtcConnectEngine::closeMeeting()
	{
		Message byeReq;
		byeReq.set_from(userInfo.userId);
		byeReq.set_to(meetingId);
		byeReq.set_cseq(cseq++);
		byeReq.set_call_id(std::to_string(rand()));
		byeReq.set_meth("CLOSE");
		byeReq.set_isresponse(false);
		I_LOG("CLOSE Req: {}", byeReq.js.dump(4));
		oatpp::String js = oatpp::String(byeReq.js.dump());
		sendSocket(js);
	...
	void RtcConnectEngine::onOK(Message resp) {
		...
		else if (resp.cmeth() == "CREATE") {
			I_LOG("CREATE onOK");
			meetingId = resp.meetingId();
			OnScheduleMeeting(meetingId);
		}
		else if (resp.cmeth() == "CLOSE") {
			I_LOG("CLOSE onOK");
			if (Closeflag == false) {
				onClose();
			}
	...
	void RtcConnectEngine::keepalive() {
		...
		try {
			while (!threadDestroy) {
				...
				Sleep(2000);
				//if (noHeartbeatRespTime == 0)
				//	noHeartbeatRespTime = seeker::Time::currentTime();
				//if (seeker::Time::currentTime()-noHeartbeatRespTime>3000) {
				//	W_LOG("no receive timeout 3s");
				//	signalingSocket->stopListening();
				//	signalingSocket = nullptr;
				//	noHeartbeatRespTime = 0;
				//	//signalState = State::NONE;
				//	if (reconnectThread.joinable()) 
				//		reconnectThread.join();
				//	std::thread loop{ &RtcConnectEngine::onReconnect, this };
	...
	bool RtcConnectEngine::reconnect()
	{
		auto connectionProvider = oatpp::network::tcp::client::ConnectionProvider::createShared({ signalInfo.signalIp, signalInfo.signalPort });
		auto connector = oatpp::websocket::Connector::createShared(connectionProvider);
		try {
			auto connection = connector->connect("/connectWS");
			if (connection) {
				...
				signalingSocket = nullptr;
				signalingSocket = oatpp::websocket::WebSocket::createShared(connection, true);
			}
			else {
				return false;
			}
	...
	void RtcConnectEngine::getMediaInfo(MediaInfo& mediaInfo)
	{
	...
	void RtcConnectEngine::onReconnect() {
		...
		while (seeker::time::currentTime() - timepoint <= 30000&&signalState==State::NONE) {
			while (!reconnect()&&seeker::time::currentTime() - timepoint <= 30000) {
				I_LOG("Reconnect ip {} port {} continue", signalInfo.signalIp, signalInfo.signalPort);
			}
			if (signalState == State::NONE) {
				break;
			}
			try {
				if (lastsignalState == State::LOGIN_ON) {
					//connect(signalInfo.signalIp, signalInfo.signalPort);
					login(userInfo.userId, userInfo.password);
				}
				else if (lastsignalState == State::MEETING) {
					//connect(signalInfo.signalIp, signalInfo.signalPort);
					login(userInfo.userId, userInfo.password);
		...
		if (signalState == State::NONE) {
			E_LOG("reconnect ip [{}] port [{}] falied", signalInfo.signalIp, signalInfo.signalPort);
			OnReconnectFailure();
			return;
		}
		D_LOG("reconnect ip [{}] port [{}] success", signalInfo.signalIp, signalInfo.signalPort);
		OnReconnectSuccess();
		return;
	}
}
