...
struct SeqCounter {
  ...
  uint32_t lost() { return packetExpected_ - seqOk_ - seqLate_ - seqGap_; };
  float lostRate() { return static_cast<float>(lost()) / packetExpected_; };
  void reset() {
    lastSeq = -1;
    packetCount_ = 0;
    packetExpected_ = 0;
...
class RtpPipe {
 ...
 public:
  RtpPipe(uint16_t reorderMaxSize)
      : reorderSize{reorderMaxSize},
 ...
 private:
  ...
  int takeRtps(std::queue<RawData>& outQueue, uint16_t forceRange) {
    int rst = 0;
    if (headPos == tailPos) {
      return rst;
    }
    ...
    for (uint16_t i = 0; i < forceRange && headPos != tailPos; i++) {
      if (seqArray[tailPos]) {
        ...
        outQueue.emplace(std::move(rtpDataArray[tailPos]));
      }
      goForward(tailPos, 1);
      forwardCount += 1;
    }
...
inline SeqState RtpPipe::updateData(uint16_t seq, std::vector<uint8_t>&& rtpData,
  ...
  T_LOG("updateData begin: head={} tail={}", headPos, tailPos);

  seqCounter.packetCount_ += 1;

  if (seqCounter.lastSeq < 0) {
    outQueue.emplace(std::move(rtpData));
    setSeqState(1);
    goForward(tailPos, 1);
    seqCounter.seqOk_ += 1;
  ...
  T_LOG("in updateData lastSeq={} head={} tail={} diff={}", seqCounter.lastSeq, headPos,
        tailPos, diff);
  ...
  if (diff == 1) {
    ...
    if (tailPos == headPos) {
      ...
      T_LOG("store data 0: pos={} size={}", headPos, rtpDataArray[headPos].size());
      setSeqState(1);

      int d1 = forwardDistance(headPos, tailPos);
      int forceRange = d1 - reorderSize;
      T_LOG("d1={} forceRange={}", d1, forceRange);

      if (forceRange > 0) {
        int rst = takeRtps(outQueue, forceRange);
        if (rst < 0) {
          E_LOG("error: state={} diff={} seq={} lastSeq={} d1={} forceRange={}", (int)state,
                diff, seq, seqCounter.lastSeq, d1, forceRange);
          exit(0);
        }
      }
    }
    ...
    rtpDataArray[headPos].swap(rtpData);
    T_LOG("store data 1: pos={} size={}", headPos, rtpDataArray[headPos].size());
    setSeqState(1);
    ...
    // ONLY FOR DEBUG
    //T_LOG("jump: state={} diff={} seq={} lastSeq={} d1={} forceRange={}", (int)state, diff,
    //      seq, seqCounter.lastSeq, d1, forceRange);
    ...
    if (rst < 0) {
      E_LOG("error: state={} diff={} seq={} lastSeq={} reorderSize={}", (int)state, diff, seq,
            seqCounter.lastSeq, reorderSize);
      exit(0);
    }
    resetPos();
    ...
    D_LOG("SeqState seq={} lastSeq={} diff={}", seq, seqCounter.lastSeq, diff);
    state = SeqState::duplicated;
    D_LOG("SeqState duplicated1: seq={}", seq);
  } else if (diff < 0 && diff >= -reorderSize) {
    uint16_t pos = getPos(diff);
    T_LOG("SeqState seq={} lastSeq={} diff={} pos={}", seq, seqCounter.lastSeq, diff, pos);
    if (seqArray[pos]) {
      state = SeqState::duplicated;
      D_LOG("SeqState duplicated2: seq={}", seq);
    } else {
      int inRtpDistance = -diff;
      uint16_t currentPos = back(headPos, 1);
      int tailDistance = headPos == tailPos ? -1 : forwardDistance(currentPos, tailPos);
      T_LOG("store: inRtpDistance={} tailDistance={}", inRtpDistance, tailDistance);

      if (inRtpDistance <= tailDistance) {
        rtpDataArray[pos].swap(rtpData);
        T_LOG("late rtp store data 1: pos={} size={}", pos, rtpDataArray[pos].size());
        seqArray[pos] = 1;
        state = SeqState::late;
        T_LOG("SeqState late: seq={}", seq);
        takeRtps(outQueue, 0);
        T_LOG("SeqState late:  --- 2 --- size={}", outQueue.size());
      } else {
        state = SeqState::old;
        T_LOG("SeqState old1");
      }
    ...
    resetPos();
    // FOR DEBUG
    T_LOG("error: state={} diff={} seq={} lastSeq={}", (int)state, diff, seq,
          seqCounter.lastSeq);
  }
  ...
  T_LOG("updateData end: head={} tail={}", headPos, tailPos);
  return state;
}
