...
@author Tao Zhang
@since 2023/7/24
@version 0.0.1-SNAPSHOT 2023/7/31
*/
#pragma once
...
inline RtcpType findRtcpType(const std::vector<uint8_t>& rtcpData) {
  ...
  if (pt == 192 || pt == 193 || (pt >= 200 && pt <= 207)) {
    return RtcpType{pt};
  } else {
    return RtcpType::UNKNOWN;
  }
...
inline uint8_t getFmtValue(const uint8_t* rtcpHeader) {
  uint8_t firstByte = rtcpHeader[0];
  uint8_t fmtValue = (firstByte >> 0) & 0x1F;
  return fmtValue;
}
