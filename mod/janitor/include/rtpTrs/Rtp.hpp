...
class Rtp {
 ...
 public:
  ...
  void getHeaderExtension(std::vector<uint8_t>& buf) {
  ...
  uint32_t ssrc() { return getSsrc(udpBuff.data()); }
  ...
  uint8_t payloadType() { return getPayloadType(udpBuff.data()); };

  bool marker() { return getMark(udpBuff.data()); };

  uint16_t seq() { return getSeq(udpBuff.data()); };

  void updateSeq(uint16_t seq) { return setSeq(udpBuff.data(), seq); };

  bool hasHeaderExtension() { return hasExtension(udpBuff.data()); };
