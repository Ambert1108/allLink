...
class RtpTransceiver {
 ...
 public:  // API
   ...
   * 绑定本地UDP地址(ipv6和端口)
   * - 返回值为0, 表示成功, 其他则表示绑定失败.
   * - `localIp`: 要绑定的本地IPV6地址
   * - `localPort`: 要绑定的UDP端口
   * - `sendOnly`: 如果为 true, 则将跳过所有接收流程，不提供接收能力
   * - `localRtcpPort`: 如果为 localRtcpPort > 0, 则启动Rtcp收发能力
   * - 绑定成功之后, `transceiver`就自动开始接收绑定端口发来的数据了.
   */
  inline int openV6(const std::string& localIpv6, uint16_t localRtpPort,
                    uint16_t localRtcpPort = 0, bool sendOnly = false);
 ...
 private:
  ...
  OneOnePipe<Received<RawData>> rtpRecvBuffQueue{60};
  OneOnePipe<Received<RawData>> rtcpRecvBuffQueue{60};
      ...
      if (!recvUdpOnly && Rtp::isValidRtp(udp)) {
        ...
        if (currentPipe != nullptr && currentStream.ssrc == lastSsrc &&
          ...
          if (it != rtpPipeMap.end()) {
            ...
            auto newIt = rtpPipeMap.try_emplace(currentStream, reorderSize_);
            if (newIt.second) {
...
void RtpTransceiver::recvWorker() {
  ...
  std::queue<RawData> rtpTmpQueue{};
  std::queue<RawData> rtcpTmpQueue{};
    ...
    if (recvUdpOnly) {
      ...
      while (!rtpWorkingQueue.empty()) {
        ...
        if (!Rtp::isValidRtp(udp)) {
          ...
          currentPipe->reorderRtp(std::move(udp), seeker::time::currentTime(), rtpTmpQueue);
...
void RtpTransceiver::RtpProcessor::pushUdp(RawData&& udp, const udp::endpoint& from) {
  if (p.sendOnly_) {
  ...
  if (!success) {
    E_LOG("rtpRecvBuffQueue enqueue error! size={}", p.rtpRecvBuffQueue.size());
  } else {
    p.recvBuffCv.notify_one();
  }
}

void RtpTransceiver::RtcpProcessor::pushUdp(RawData&& udp, const udp::endpoint& from) {
  if (p.sendOnly_) {
    std::string msg = fmt::format("traceId[{}], pushUdp: sendOnly.", p.traceId_);
    E_LOG(msg);
    throw std::logic_error(msg);
  ...
  while (pos + 4 <= udpLen) {
    ...
    if (pos + rtcpLengthInBytes <= udpLen) {
      ...
      W_LOG("rtcp split error. pos={} rtcpLength={} rtcpLengthInBytes={} udpLen={}", pos,
            rtcpLength, rtcpLengthInBytes, udpLen);
    }
