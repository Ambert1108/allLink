...
namespace seeker {
  ...
  public:
    static std::string base64Decode(std::string& sdp_sps) {
      std::string base64_chars =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
      std::string ret;
      int val = 0, valb = -8;
      for (unsigned char c : sdp_sps) {
        if (c == '=') {
          break;
        }
        ...
        val = (static_cast<unsigned long long>(val) << 6) + pos;
        valb += 6;
        if (valb >= 0) {
    ...
    static int parseSpsInfo(const std::string& sdp, std::string& sps) {
      int pos = sdp.find("sprop-parameter-sets=");
      if (pos == -1) {
        E_LOG("sdp not find sprop-parameter-sets");
        return -1;
      }
      std::string sdp_sps = sdp.substr(
        static_cast<std::basic_string<char, std::char_traits<char>, std::allocator<char>>::size_type>(pos) + 21);

      int pos1, pos2 = sdp.find("sprop-parameter-sets=");
      pos1 = sdp_sps.find("a=");
      pos2 = sdp_sps.find(";");
      pos = pos1 < pos2 ? pos1 : pos2;

      if (pos == -1) {
        E_LOG("sdp or a= not find");
        return -1;
      }
      sdp_sps = sdp_sps.substr(0, pos);

      sps = base64Decode(sdp_sps);

      return 0;
    }

    static bool getSpsInfo(BYTE* buf, UINT nLen, int& width, int& height, int& fps) {
      UINT StartBit = 0;
      fps = 0;
      ...
      if (nal_unit_type == 7) {
        ...
        if (vui_parameter_present_flag) {
          ...
          if (timing_info_present_flag) {
            ...
            I_LOG("[Debug] ParseSps.h num_units_in_tick={}", num_units_in_tick);
            int time_scale = u(32, buf, StartBit);
            if (num_units_in_tick != 0) fps = time_scale / num_units_in_tick;
            else fps = 0;
