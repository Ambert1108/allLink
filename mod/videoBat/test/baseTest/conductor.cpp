...
bool Conductor::InitializePeerConnection() {
  ...
  if (!peer_connection_factory_) {
    E_LOG("Failed to initialize PeerConnectionFactory");
    DeletePeerConnection();
    return false;
  }
  ...
  if (!CreatePeerConnection()) {
    E_LOG("CreatePeerConnection failed");
    DeletePeerConnection();
    return false;
  }
...
void Conductor::OnIceCandidate(const webrtc::IceCandidateInterface* candidate) {
  ...
  if (loopback_) {
    if (!peer_connection_->AddIceCandidate(candidate)) {
      E_LOG("Failed to apply the received candidate");
    }
    return;
  }
...
void Conductor::OnMessageFromPeer(int peer_id, const std::string& message) { 
  ...
  I_LOG("peer_id {} message {}", peer_id, message);
  ...
  if (!peer_connection_.get()) {
    ...
    RTC_DCHECK(peer_id_ != -1);
    W_LOG("Received a message from unknown peer while already in a conversation with a different peer.");
    return;
  }
  ...
  if (!type_str.empty()) {
    ...
    if (!session_description) {
      W_LOG("SdpParseError was:{}", error.description);
      return;
    }
    I_LOG("Received session description:{}", message);
    ...
    I_LOG("Received candidate:{}", message);
  }
}
...
void Conductor::AddTracks() {
  ...
  if (!result_or_error.ok()) {
    E_LOG("Failed to add audio track to PeerConnection:");
  }
...
void Conductor::UIThreadCallback(int msg_id, void* data) {
  ...
  case SEND_MESSAGE_TO_PEER: {
    I_LOG("SEND_MESSAGE_TO_PEER");
    std::string* msg = reinterpret_cast<std::string*>(data);
    if (msg) {
      I_LOG("msg_id: {},msg: {}", msg_id, *msg);
