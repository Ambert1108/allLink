...
void PeerConnectionClient::Connect(const std::string& server, int port, const std::string& client_name) {
  ...
  if (state_ != NOT_CONNECTED) {
    W_LOG("The client must not be connected before you can call Connect()");
...
bool PeerConnectionClient::SignOut() {
  ...
  if (control_socket_->GetState() == rtc::Socket::CS_CLOSED) {
    ...
    if (my_id_ != -1) {
      ...
      return true;  // Can occur if the app is closed before we finish connecting.
    }
  }
  else {
...
void PeerConnectionClient::OnMessageFromPeer(int peer_id, const std::string& message) {
  if (message.length() == (sizeof(kByeMessage) - 1) && message.compare(kByeMessage) == 0) {
    std::cout << "Peer " << peer_id << " has disconnected." << std::endl;
    callback_->OnPeerDisconnected(peer_id);
  }
  else {
    std::cout << "Message from peer " << peer_id << ": " << message << std::endl;
    ...
    if (GetHeaderValue(*data, i, "\r\nContent-Length: ", content_length)) {
      ...
      if (data->length() >= total_response_size) {
        ...
        if (GetHeaderValue(*data, i, kConnection, &should_close) && should_close.compare("close") == 0) {
          socket->Close();
          OnClose(socket, 0); // Notify closure
        }
      }
    }
    else {
      std::cerr << "No content length field specified by the server." << std::endl;
    }
  }
  return ret;
}
        ...
        if (content_length) {
          ...
          while (pos < control_data_.size()) {
            ...
            if (ParseEntry(control_data_.substr(pos, eol - pos), &name, &id, &connected) && id != my_id_) {
              peers_[id] = name;
              std::cout << "Peer connected: " << name << " (ID: " << id << ")" << std::endl;
...
void PeerConnectionClient::OnHangingGetRead(rtc::Socket* socket) {
  std::cout << "Hanging GET read event." << std::endl;
  size_t content_length = 0;
      ...
      if (my_id_ == static_cast<int>(peer_id)) {
        ...
        if (ParseEntry(notification_data_.substr(pos), &name, &id, &connected)) {
          if (connected) {
            ...
            peers_.erase(id);
            std::cout << "Peer disconnected: " << id << std::endl;
...
void PeerConnectionClient::OnClose(rtc::Socket* socket, int err) {
  std::cout << "Socket closed with error: " << err << std::endl;
  ...
  if (err != ECONNREFUSED) {
    ...
    if (socket == control_socket_.get()) {
      std::cerr << "Connection refused; retrying in 2 seconds" << std::endl;
