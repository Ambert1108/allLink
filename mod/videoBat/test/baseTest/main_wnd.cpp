...
void MainWnd::SwitchToPeerList(const Peers& peers) {
  ...
  I_LOG("SwitchToPeerList");
  Peers::const_iterator i = peers.begin();
  for (; i != peers.end(); ++i)
    I_LOG("peerId={},name={}", i->first, i->second);
...
bool MainWnd::OnMessage(UINT msg, WPARAM wp, LPARAM lp, LRESULT* result) {
  ...
  case WM_ERASEBKGND:
    *result = TRUE;
    ...
    //case WM_PAINT:
    //  OnPaint();//ÊÓÆµÁ÷µÄÖ¡äÖÈ¾¼°Í¼ÐÎ½çÃæäÖÈ¾
    //  return true;

    //case WM_SETFOCUS://±à¼­¿ò¾Û½¹¶¨Î»
    //  if (ui_ == CONNECT_TO_SERVER) {
    //    SetFocus(edit1_);
    //  } else if (ui_ == LIST_PEERS) {
    //    SetFocus(listbox_);
