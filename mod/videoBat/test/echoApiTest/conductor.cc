...
void Conductor::AddTracks() {
    ...
    int result = videoEngine.addVideoTrack(peer_connection_factory_, peer_connection_, video_track_);
    if (result) {
      E_LOG("addVideoTrack error!");
    }
    ...
    //}
    main_wnd_->SwitchToStreamingUI();//跳转到通话状态
    I_LOG("AddTracks end");
}
...
void Conductor::createSession() {
   ...
   // I_LOG("httpCallAttch req_body {}", j.dump());
    ...
    // 在回复中 data字段获取handleId
    handleId = j["data"]["id"];

    //发送message 信息，携带 sessionId, handleId
    sendMessageToJanus();
    // 初始化 peerConnection
...
void Conductor::keepAlive() {
    ...
    while (true) {
        ...
        keepCount++;
        I_LOG("count = {}", keepCount);
        if (keepCount % 2) {
          //switchCamera(false);
          auto cameraMap = videoEngine.getCameraMap();
          for (auto cam : cameraMap) {
            I_LOG("id = {}, cam = {}", cam.first, cam.second);
          }
        }
        else {
          videoEngine.StartScreenCapture();
          //switchCamera(true);
        }
        if (keepCount == 2) {
          //rtc::scoped_refptr<webrtc::VideoTrackInterface> video_track;
          //videoEngine.setCamera(1, video_track);
          //resetRender(video_track);
        }
    }
    return;
}
...
void Conductor::ConnectToPeer(int peer_id) {
  I_LOG("ConnectToPeer {} ", peer_id);
  RTC_DCHECK(peer_id_ == -1);
  RTC_DCHECK(peer_id != -1);

  if (peer_connection_.get()) {
      E_LOG("We only support connecting to one peer at a time");
    return;
  }
  ...
  if (InitializePeerConnection()) {
    ...
    peer_connection_->CreateOffer(
      ...
      E_LOG("Failed to initialize PeerConnection");
  }
}
...
void Conductor::resetRender(rtc::scoped_refptr<webrtc::VideoTrackInterface> track_) {
  main_wnd_->StartRemoteRenderer(track_.get());
}

void Conductor::UIThreadCallback(int msg_id, void* data) {
  I_LOG("UIThreadCallback");
  switch (msg_id) {
    case NEW_TRACK_ADDED: {
        I_LOG("NEW_TRACK_ADDED");
...
void Conductor::switchCamera(bool flag) {
   videoEngine.switchCamera(flag);
}