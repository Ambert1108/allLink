...
void Conductor::OnMessageFromPeer(int peer_id, const std::string& message) {
  I_LOG("OnMessageFromPeer peer_id {} message {}", peer_id, message);
  ...
  if (!peer_connection_.get()) {
    ...
    E_LOG("conversation with a different peer.");
    return;
  }
  ...
  if (!type_str.empty()) {
  ...
  else {//连接时进入，生成CreateIceCandidate
    std::string sdp_mid;
    ...
    webrtc::SdpParseError error;  E_LOG("111");
...
void Conductor::UIThreadCallback(int msg_id, void* data) {
  ...
  switch (msg_id) {
    ...
    case SEND_MESSAGE_TO_PEER: {
      ...
      if (msg) {
        I_LOG("SEND_MESSAGE_TO_PEER {} {}", msg_id, *msg);
