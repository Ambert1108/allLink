...
void PeerConnectionClient::DoConnect() {
    I_LOG("DoConnect");
  control_socket_.reset(CreateClientSocket(server_address_.ipaddr().family()));//创建一个控制连接（发送和接收命令）
  hanging_get_.reset(CreateClientSocket(server_address_.ipaddr().family()));//用于 hanging GET 操作（长轮询，用于接收服务器的实时更新）
  InitSocketSignals(); //初始化套接字信号，包括连接、数据接收等事件的回调处理。
  char buffer[1024]; //准备一个 HTTP GET 请求，用于登录到服务器。这个请求的路径是 "/sign_in"，并且包含一个查询参数，即客户端的名字。
  snprintf(buffer, sizeof(buffer), "GET /sign_in?%s HTTP/1.0\r\n\r\n",
           client_name_.c_str());
  onconnect_data_ = buffer;

  bool ret = ConnectControlSocket();//尝试连接到控制套接字。如果连接成功，ConnectControlSocket() 将返回 true，否则返回 false。
  if (ret)
    state_ = SIGNING_IN;
...
bool PeerConnectionClient::SendToPeer(int peer_id, const std::string& message) {
    I_LOG("SendToPeer peer_id {} {}", peer_id, message);
  if (state_ != CONNECTED)
    return false;
...
bool PeerConnectionClient::ConnectControlSocket() {
  RTC_DCHECK(control_socket_->GetState() == rtc::Socket::CS_CLOSED);//检查当前的连接状态
  int err = control_socket_->Connect(server_address_);//向信令服务器发起连接请求
  if (err == SOCKET_ERROR) {
    Close();
    return false;
  }
...
void PeerConnectionClient::OnConnect(rtc::Socket* socket) {
  RTC_DCHECK(!onconnect_data_.empty());//判断发送的信令是否为空
  size_t sent = socket->Send(onconnect_data_.c_str(), onconnect_data_.length());//发送
...
void PeerConnectionClient::OnMessageFromPeer(int peer_id,
    ...
    I_LOG("OnMessageFromPeer {} {}", peer_id, message);
