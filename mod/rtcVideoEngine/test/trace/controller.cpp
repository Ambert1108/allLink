...
namespace wt {
  Controller::Controller(MediaClient* client, MainWindow* main_wnd)
    : peer_id_(-1), loopback_(false), isReceive_(false), client_(client), main_wnd_(main_wnd) {
  ...
  bool Controller::connection_active() const {
  ...
  bool Controller::InitializePeerConnection() {
    ...
    if (!peer_connection_factory_) {
      E_LOG("Error, Failed to initialize PeerConnectionFactory\n");
      DeletePeerConnection();
      return false;
    }

    if (!CreatePeerConnection()) {
      E_LOG("Error, CreatePeerConnection failed\n");
      DeletePeerConnection();
      return false;
    }
  ...
  bool Controller::ReinitializePeerConnectionForLoopback() {
    loopback_ = true;
  ...
  void Controller::OnAddTrack(
    ...
    D_LOG("on add track, id:{}", receiver->id());
    main_wnd_->QueueUIThreadCallback(NEW_TRACK_ADDED,
      receiver->track().release());
  }

  void Controller::OnRemoveTrack(
    rtc::scoped_refptr<webrtc::RtpReceiverInterface> receiver) {
    RTC_LOG(LS_INFO) << __FUNCTION__ << " " << receiver->id();
  ...
  void Controller::OnMessageFromPeer(int peer_id, const std::string& message) {
    ...
    if (!reader->parse(message.data(), message.data() + message.length(),
      &jmessage, nullptr)) {
      W_LOG("Received unknown message. {}", message);
      return;
    }
    std::string type_str;
    ...
    if (!type_str.empty()) {
      ...
      D_LOG(" Received session description :{}", message);
      ...
      D_LOG(" Received candidate :{}", message);
    }
    D_LOG("on peer 7");
  }
  ...
  void Controller::OnServerConnectionFailure() {
    ...
    std::cerr << "Error, Failed to connect to " << server_ << std::endl;
  }
  ...
  void Controller::ConnectToPeer(int peer_id) {
    ...
    if (peer_connection_.get()) {
      ...
      std::cerr << "Error, We only support connecting to one peer at a time" << std::endl;
      return;
    }
    isReceive_ = false;
    if (InitializePeerConnection()) {
      ...
      std::cerr << "Error, Failed to initialize PeerConnection" << std::endl;
    }
  }
  ...
  void Controller::AddTracks() {
    ...
    main_wnd_->SwitchToStreamingUI(isReceive_);
  }
  ...
  void Controller::UIThreadCallback(int msg_id, void* data) {
    switch (msg_id) {
    ...
    case NEW_TRACK_ADDED: {
      ...
      D_LOG("new track try add");
      if (track->kind() == webrtc::MediaStreamTrackInterface::kVideoKind) {
        D_LOG("add video track");
  ...
  void Controller::sendMessage(const std::string& json_object) {
