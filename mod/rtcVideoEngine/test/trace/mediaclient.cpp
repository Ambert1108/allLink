...
namespace wt {
  ...
  void MediaClient::InitSocketSignals() {
    ...
    control_socket_->SignalCloseEvent.connect(this,
      &MediaClient::OnClose);
    hanging_get_->SignalCloseEvent.connect(this, &MediaClient::OnClose);

    /*注册connect事件槽函数*/
    control_socket_->SignalConnectEvent.connect(this,
      &MediaClient::OnConnect);
    hanging_get_->SignalConnectEvent.connect(
      this, &MediaClient::OnHangingGetConnect);

    /*注册read事件槽函数*/
    control_socket_->SignalReadEvent.connect(this, &MediaClient::OnRead);
    hanging_get_->SignalReadEvent.connect(
      this, &MediaClient::OnHangingGetRead);
  }

  int MediaClient::id() const {
    return my_id_;
  }

  bool MediaClient::is_connected() const {
    return my_id_ != -1;
  }

  const Peers& MediaClient::peers() const {
    return peers_;
  }

  void MediaClient::RegisterObserver(
    MediaClientObserver* callback) {
    RTC_DCHECK(!callback_);
    callback_ = callback; /*保存订阅者*/
  }

  void MediaClient::Connect(const std::string& server,
    int port,
    ...
    if (server_address_.IsUnresolvedIP()) {
      ...
      resolver_ = std::make_unique<webrtc::AsyncDnsResolver>();   /*创建一个异步的域名解析器*/
      resolver_->Start(server_address_,
        [this] { OnResolveResult(resolver_->result()); }); /*注册信号，域名解析结束后会回调这个处理函数；开始解析*/
    }
  ...
  void MediaClient::OnResolveResult(
    const webrtc::AsyncDnsResolverResult& result) {
    if (result.GetError() != 0) { /*没有解析成功*/
      callback_->OnServerConnectionFailure();
      resolver_.reset();
      state_ = NOT_CONNECTED; /*设置为非连接状态*/
      return;
    }
    if (!result.GetResolvedAddress(AF_INET, &server_address_)) { /*获取解析后的地址*/
      callback_->OnServerConnectionFailure();
      resolver_.reset();
  ...
  void MediaClient::DoConnect() {
    /*创建异步socket*/
    control_socket_.reset(CreateClientSocket(server_address_.ipaddr().family())); /*用于主动的向信令服务器发送信令*/
    hanging_get_.reset(CreateClientSocket(server_address_.ipaddr().family()));    /*用于向信令服务器请求信令消息，每次都向信令服务器发送wait信令，等待信令服务器响应*/
  ...
  bool MediaClient::SendToPeer(int peer_id, const std::string& message) {
    if (state_ != CONNECTED)
      return false;
  ...
  bool MediaClient::SendHangUp(int peer_id) {
    return SendToPeer(peer_id, kByeMessage);
  }

  bool MediaClient::IsSendingMessage() {
    return state_ == CONNECTED &&
      control_socket_->GetState() != rtc::Socket::CS_CLOSED;
  }

  bool MediaClient::SignOut() {
    D_LOG("sign out");
  ...
  bool MediaClient::ConnectControlSocket() {
    /*检查socket的连接状态*/
    RTC_DCHECK(control_socket_->GetState() == rtc::Socket::CS_CLOSED);
  ...
  void MediaClient::OnConnect(rtc::Socket* socket) {
  ...
  void MediaClient::OnHangingGetConnect(rtc::Socket* socket) {
    char buffer[1024];
  ...
  void MediaClient::OnMessageFromPeer(int peer_id,
    ...
    if (message.length() == (sizeof(kByeMessage) - 1) &&
      ...
      D_LOG("handle offer 2");
      /*offer、answer、candidate信息需要发送给Conductor::OnMessageFromPeer()*/
      callback_->OnMessageFromPeer(peer_id, message);
      D_LOG("handle offer finish");
    }
  }

  bool MediaClient::GetHeaderValue(const std::string& data,
    size_t eoh,
  ...
  bool MediaClient::ReadIntoBuffer(rtc::Socket* socket,
    std::string* data,
  ...
  void MediaClient::OnRead(rtc::Socket* socket) {
  ...
  void MediaClient::OnHangingGetRead(rtc::Socket* socket) {
  ...
  bool MediaClient::ParseEntry(const std::string& entry,
    std::string* name,
    int* id,
    bool* connected) {
  ...
  int MediaClient::GetResponseStatus(const std::string& response) {
    int status = -1;
  ...
  bool MediaClient::ParseServerResponse(const std::string& response, size_t content_length, 
  ...
  void MediaClient::OnClose(rtc::Socket* socket, int err) {
    RTC_LOG(LS_INFO) << __FUNCTION__;
